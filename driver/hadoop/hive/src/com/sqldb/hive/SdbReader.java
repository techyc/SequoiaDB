...
import com.sqldb.util.logger;

class SqlDBRecord {
	private byte[] stream = null;
	private int  length = 0;
	
	public SqlDBRecord(byte[] stream, int length) {
		this.stream = stream;
		this.length = length;
	}
...
public class SdbReader extends Thread implements
	...
	BlockingQueue<SqlDBRecord> queue = new ArrayBlockingQueue<SqlDBRecord>(1024);
	...
	public void run() {
		...
		while (cursor.hasNextRaw()) {
			try {
				...
				this.queue.put(new SqlDBRecord(recordWithAllColumns, pos));
				
			} catch (Exception e) {
				LOG.error("Failed to get a record from sqldb.", e);
				continue;
			}
		}
		
		try {
			this.queue.put(new SqlDBRecord(null, -1));

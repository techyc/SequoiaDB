...
public class SequoiadbWriter<K, V> extends RecordWriter<K, V> {
	...
	public SequoiadbWriter(String collectionSpaceName, String collectionName,
		...
		super();
		this.sqldb = new Sequoiadb(sdbConnAddr.getHost(),
		...
		if(sqldb.isCollectionSpaceExist(collectionSpaceName)){
			space = sqldb.getCollectionSpace(collectionSpaceName);	
		}else{
			sqldb.createCollectionSpace(collectionSpaceName);
		}

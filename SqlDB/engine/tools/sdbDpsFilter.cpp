...
namespace
{
                           ...
                           CHAR *pOutBuffer, const UINT64 outBufferSize )
   {
      ...
      len += ossSnprintf( pOutBuffer + len, outBufferSize - len,
                         ...
                         metaData.fileCount ) ;
      len += ossSnprintf( pOutBuffer + len, outBufferSize - len,
                         "    LogFile begin     : sqldbLog.%d"OSS_NEWLINE,
                         metaData.fileBegin ) ;
      len += ossSnprintf( pOutBuffer + len, outBufferSize - len,
                         "    LogFile work      : sqldbLog.%d"OSS_NEWLINE,
                         metaData.fileWork ) ;
      len += ossSnprintf( pOutBuffer + len, outBufferSize - len,
      ...
      for( idx = 0; idx < metaData.metaList.size(); ++idx )
      {
         const dpsFileMeta& meta = metaData.metaList[ idx ] ;
         len += ossSnprintf( pOutBuffer + len, outBufferSize - len,
                OSS_NEWLINE"Log File Name: sqldbLog.%d"OSS_NEWLINE,
                meta.index ) ;
         len += ossSnprintf( pOutBuffer + len, outBufferSize - len,
                               ...
                               const CHAR *logFilePath )
{
   ...
   if( dpsLogFilter::isDir( data->srcPath ) )
   {
      ...
      for( INT32 idx = 0 ; idx < MAX_FILE_COUNT ; ++idx )
      {
         ...
         ossSnprintf( filename, OSS_MAX_PATHSIZE, "%s/sqldbLog.%d",
                      filepath, idx ) ;

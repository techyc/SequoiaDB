...
namespace engine
{
   ...
   */
   ...
   #define ENGINE_NPIPE_PREFIX         "sqldb_engine_"
   #if defined (_LINUX)
   #define PROC_PATH                   "/proc"
   #define PROC_CMDLINE_PATH_FORMAT    PROC_PATH"/%s/cmdline"
   #define ENGINE_NAME                 "sqldb"
   #define ENGINE_NPIPE_PREFIX_BW      "sqldb_engine_bw_"
   #elif defined (_WINDOWS)
   #define ENGINE_NAME                 "sqldb.exe"
   ...
   */
   enum SDB_TYPE
   {
      SDB_TYPE_DB  = 1,    // sqldb: data, standalone, coord, catalog
      SDB_TYPE_OM,         // om

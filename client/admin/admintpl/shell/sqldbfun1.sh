...
function sshCheckEnv()
{
   ...
   if [ "${localHostname}" = "${1}" ]; then
      ...
      ssh root@$1 "/tmp/sqldbsub.sh 1 ${2} ${3} ${4} ${5}"
      if [ $? -ne 0 ]; then
         return 1
      else
         return 0
...
function sshNodeStart()
{
   ...
   if [ "${localHostname}" = "${1}" ]; then
      ...
      ssh root@$1 "/tmp/sqldbsub.sh 3 ${2} ${3} ${4} ${5}"
      if [ $? -ne 0 ]; then
         return 1
      else
         return 0
...
function sshInstallon()
{
   ...
   if [ "${localHostname}" = "${1}" ]; then
      ...
      ssh root@${1} "/tmp/sqldbsub.sh 2 ${2}"
      if [ $? -ne 0 ]; then
         return 1
      else
         return 0
...
function sshRevokeTask()
{
   local localHostname=`hostname`
   if [ "${localHostname}" = "${1}" ]; then
      fileexit=`if [ -f "/tmp/sqldbsub.sh" ]; then echo '1'; else echo '0'; fi`
      if [ $? -ne 1 ]; then
         #不存在
         return 0
      fi
      ...
      fileexit=`ssh root@${1} "if [ -f "/tmp/sqldbsub.sh" ]; then echo '1'; else echo '0'; fi"`
      if [ $? -ne 1 ]; then
         #不存在
         return 0
      fi
      ssh root@${1} "/tmp/sqldbsub.sh 5"
      if [ $? -ne 0 ]; then
         return 1
      else
         return 0
...
function sshGroupStart()
{
   ...
   if [ "${localHostname}" = "${2}" ]; then
      ...
      ssh root@${2} "/tmp/sqldbsub.sh 4 ${1} ${2} ${3}"
      if [ $? -ne 0 ]; then
         return 1
      else
         return 0
...
function sshOneGroupStart()
{
   ...
   if [ "${localHostname}" = "${2}" ]; then
      ...
      ssh root@${2} "/tmp/sqldbsub.sh 6 ${1} ${2} ${3} ${4}"
      if [ $? -ne 0 ]; then
         return 1
      else
         return 0
...
#复制文件到其他主机上的/tmp目录
#参数1 文件名 例如 "/opt/sqldb/sqldb.run"
...
function checkEnvOneHost()
{
   ...
   if [ "${type}" = "1" ]; then
      ...
      #复制子程序到目标机器
      copyFile2OtherHost "${PWD}/sqldbconfig.sh" ${target}
      if [ $? -ne 0 ]; then
         echo_r "Error" $FUNCNAME $LINENO "Copy the sqldbconfig.sh file to the ${target} failure"
         return 1
      fi
      changeFilePower "${target}" "/tmp/sqldbconfig.sh"

      copyFile2OtherHost "${PWD}/sqldbfun1.sh" ${target}
      if [ $? -ne 0 ]; then
         echo_r "Error" $FUNCNAME $LINENO "Copy the sqldbfun1.sh file to the ${target} failure"
         return 1
      fi
      changeFilePower "${target}" "/tmp/sqldbfun1.sh"

      copyFile2OtherHost "${PWD}/sqldbfun2.sh" ${target}
      if [ $? -ne 0 ]; then
         echo_r "Error" $FUNCNAME $LINENO "Copy the sqldbfun2.sh file to the ${target} failure"
         return 1
      fi
      changeFilePower "${target}" "/tmp/sqldbfun2.sh"

      copyFile2OtherHost "${PWD}/sqldbsub.sh" ${target}
      if [ $? -ne 0 ]; then
         echo_r "Error" $FUNCNAME $LINENO "Copy the sqldbsub.sh file to the ${target} failure"
         return 1
      fi
      changeFilePower "${target}" "/tmp/sqldbsub.sh"

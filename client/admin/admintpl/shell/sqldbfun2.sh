...
#参数1 文件名 例如 "/opt/sqldb/bin/sqldb"
...
#运行安装文件
#参数1 文件名   例如 "sqldb.run"
#参数2 安装路径 例如 "/opt/sqldb"
#参数3 用户名   例如 "sdbadmin"
#参数4 密码     例如 "sqldb"
#参数5 端口     例如 "50010"
#参数6 开机自动启动 例如 true
...
function installsdb()
{
   ...
   if [ $? -ne 0 ]; then
      echo_r "Error" $FUNCNAME $LINENO "${target} Failed to install SqlDB"
      return 1
   fi
   REVOKE_TASK_ARR=("${REVOKE_TASK_ARR[@]}" "${host_array[1]}/bin/sdbcmtop")
   return 0
